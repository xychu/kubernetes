...
var (
	...
	pcExample = templates.Examples(i18n.T(`
		...
		kubectl create priorityclass high-priority --value=1000 --description="high priority, never preempt" --preemption-policy="Never"`))
)
...
func NewCmdCreatePriorityClass(f cmdutil.Factory, ioStreams genericclioptions.IOStreams) *cobra.Command {
	...
	cmd.Flags().String("preemption-policy", string(apiv1.PreemptLowerPriority), i18n.T("preemption-policy is the policy for preempting and being preempted."))
	return cmd
}
